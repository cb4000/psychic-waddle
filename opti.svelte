...
const OptimizationComponent = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-5 bg-slate-50">
      <style jsx>{`
        .optimization-panel {
          background: white;
          border-radius: 12px;
        ...
        {activeTab === 'create' && (
          <div className="tab-content">
            <div className="variables-container">
              <div className="variable-section optimization-variables">
                <h3>Optimization Variables <span className="variable-count">(4 parameters)</span></h3>
                {optimizationData.optimizationVariables.map((variable, index) => (
                  <div key={index} className="variable-item">
                    <span className="variable-name">{variable.name}</span>
                    <span className="variable-value">{variable.currentValue}{variable.unit}</span>
                  </div>
                ))}
              </div>

              <div className="variable-section other-variables">
                <h3>Market Variables <span className="variable-count">(5 constraints)</span></h3>
                {optimizationData.otherVariables.map((variable, index) => (
                  <div key={index} className="variable-item">
                    <span className="variable-name">{variable.name}</span>
                    <span className="variable-value">{variable.currentValue}{variable.unit}</span>
                  </div>
            ...
            <div className="controls-section">
              <h3 className="controls-title">Optimization Parameters</h3>
              ...
              {historicalRuns.map((run) => (
                <div key={run.id} className="run-item">
                  <div className="run-header">
                    <h3 className="run-title">{run.title}</h3>
                  ...
                  <div className="run-details">
                    <div className="detail-item">
                      ...
                      <span className="detail-value">{run.dateRange}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">Duration:</span>
                      <span className="detail-value">{run.duration}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">
                        {run.status === 'failed' ? 'Failed:' : 'Completed:'}
                      </span>
                      <span className="detail-value">{run.completed}</span>
                    </div>
                    <div className="detail-item">
                      <span className="detail-label">
                      ...
                      <span className="detail-value">
                        {run.progress || run.error || run.result}
                      </span>
                    </div>
                  </div>
                  <div className="run-actions">
                    {run.status === 'completed' && (
                      <>
                        <button className="action-button action-view">View Results</button>
                        <button className="action-button action-download">Download</button>
                      </>
                    )}
                    {run.status === 'failed' && (
                      <button className="action-button action-view">View Logs</button>
                    )}
